...
const experience = {
  prompt: `Analyze the content below:

  [CONTENT]
  
  - Distill into 3-5 bullet points, each no more than 5 sentences.
  - **Output strictly in Traditional Chinese** unless the content is truly incomprehensible in that language. In such cases, revert to English.
  - Exclude promotional content and unrelated details.`,
  keywords: [
    'CONTENT'
  ]
}
  ...
  port.onMessage.addListener(async (request, sender, sendResponse) => {
    ...
    try {
      ...
      for (const chunk of chunks) {
        let currentAnswer = "";
        let gptQuestion = experience.prompt;
        let inputs = {
          'CONTENT': chunk
        }
        for(let keyword of experience.keywords) {
          gptQuestion = gptQuestion.replace(keyword, inputs[keyword])
        }
