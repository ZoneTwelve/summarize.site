...
function getContainer() {
  ...
  if (contentSelector && document.querySelector(contentSelector)) {
    ...
    let pageClone = selectedContainer.cloneNode(true);
    pageClone.innerHTML = DOMPurify.sanitize(
      document.head
...
function getContentOfArticle() {
  ...
  let pageClone = pageSelectedContainer.cloneNode(true);

  const pattern1 = /<a\b[^>]*>(.*?)<\/a>/gi;
  pageClone.innerHTML = DOMPurify.sanitize(
    pageClone.innerHTML.replace(pattern1, "")
  );
  const pattern2 = new RegExp("<br/?>[ \r\ns]*<br/?>", "g");
  pageClone.innerHTML = DOMPurify.sanitize(
    pageClone.innerHTML.replace(pattern2, "</p><p>")
  );

  let content = DOMPurify.sanitize(pageClone.innerHTML);
